...
struct mp4_stts_box mp4_read_stts_box(FILE *f, int size)  //level 8
{
    ...
    box.time_to_sample_table = new mp4_stts_box_sample_duration();
...
struct mp4_stsc_box mp4_read_stsc_box(FILE *f, int size)  //level 8
{
    ...
    int chunkNum = 0;
    int samplesCount = 0;
    for(int i = 0; i < box.map_amount - 1; ++i)
    {
    ...
    printf("\t\t\t\t\tsamplesCount = %d last chunk size=%d\n", samplesCount, box.scmap[box.map_amount - 1].sample_amount_in_cur_table);
    //delete box.scmap;
    return box;
}
...
struct mp4_stsz_box mp4_read_stsz_box(FILE *f, int size)  //level 8
{
    ...
    if(box.samples_size_intotal == 0){
        ...
        for(int i = 0; i < box.table_size; ++i){
            uint32_t sample_size/*box.sample_size_table[i]*/ = read_uint32_lit(f);
            
            duration += sample_size/*box.sample_size_table[i]*/;
            printf("%8u ", sample_size/*box.sample_size_table[i]*/);
            if((i+1) % 10 == 0) printf("\n");
        }
...
struct mp4_stco_box mp4_read_stco_box(FILE *f, int size)
{
    ...
    for(int i = 0 ; i < box.chunk_offset_amount; ++i) {
        uint32_t chunk_offset_from_file_begin/*box.chunk_offset_from_file_begin[i]*/ = read_uint32_lit(f);
        printf("%8x ", chunk_offset_from_file_begin/*box.chunk_offset_from_file_begin[i]*/);
        if((i+1) % 10 == 0) printf("\n");
    }
