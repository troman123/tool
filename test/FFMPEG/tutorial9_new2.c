...
int decodeThread(void *arg)
{
    ...
    for (i = 0; i < is->pFormatCtx->nb_streams; ++i)
    {
        ...
        else if (is->pFormatCtx->streams[i]->codec->codec_type == AVMEDIA_TYPE_AUDIO
            && audioIndex < 0)
        {
            audioIndex = i;
        }
        else if (is->pFormatCtx->streams[i]->codec->codec_type == AVMEDIA_TYPE_SUBTITLE)
        {
            subtitleIndex = i;
        }
    }
    ...
    streamComponentOpen(is, is->pFormatCtx, subtitleIndex);
    ...
    for (;;)
    {
        ...
        if(is->seekReq)
        {
            ...
            if (av_seek_frame(is->pFormatCtx, streamIndex, seekTarget, is->seekFlags) < 0)
            {
                ...
                fprintf(stderr, "%s: error while seeking. target: %llx,\
        ...
        {
            //fprintf(stdout,"read  subtitleStreampacket size=%d\n", packet->size);

            packetQueuePut(&is->subtitleQ, packet);
        }
        else
        {
...
int subtitleThread(void *arg)
{
    ...
    for(;;)
    {
        ...
        //fprintf(stdout, "subtitleThread pkt:%p   %d\n", pkt,
        //pkt->stream_index);
...
int videoThread(void *arg)
{
    ...
    for (;;)
    {
        ...
        //fprintf(stdout, "videoThread packet:%p   %d\n", packet, packet->stream_index);
