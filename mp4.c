 ...
 * Local prototypes
 *****************************************************************************/
static int mp4box_parse(demux_t *p_demux);
static void *mp4_thread(void *p_this);
static int   Demux_distribute ( demux_t * );
...
typedef enum mp4_parse_lock
{
    MP4_PARSE_LOCK_NORMAL,
    MP4_PARSE_LOCK_WAIT,
    MP4_PARSE_LOCK_SIGNAL,
} mp4_parse_lock;
...
enum mp4_optimized_type
{
    MP4_SPLIT_MOOV,
    MP4_DEMUX_BUFFER,
};

#define OPTIMIZED_TYPE /*(false) */(p_sys->hasInitedFirstTrack)
...
struct demux_sys_t
{
    ...
    /*extern data modify by tanzhihua*/
    bool b_optimaized;
    bool error_state;  /*has exit stat*/
    MP4_Box_t    *root_preview;
    bool hasInitedFirstTrack; /*first track is video*/
    bool hasInitedSecondTrack; /*sencond track is audio*/
    stream_t *p_stream; /*may be new stream*/
    
    Page *rawPage; /*cache raw data*/
    uint64_t rawPos;
    bool b_SkipPage;
    int pageSize;

    mp4_parse_lock parse_wait_tag;
    vlc_mutex_t parse_lock;
    vlc_cond_t  parse_cond;

    vlc_mutex_t demux_lock;
    vlc_cond_t  demux_cond;

    vlc_thread_t  thread;
    bool hasInit; /*parse mp4 box finished*/
    int demuxType; /*demux mp4 bot type*/
    int (*pf_demux)  ( demux_t * );
    block_t *p_audio_block; /*audio es block*/
    uint64_t lastSamplePos;
    bool b_demux; /*do frist demux*/
    int64_t stream_cost;

//#define MP4_IO_TEST_FP
#ifdef MP4_IO_TEST_FP
    FILE *fp;
#endif
};
...
static int MP4_AudioTrackPartInit( demux_t *p_demux, mp4_track_t *p_track,
                             MP4_Box_t *p_box_trak,
                             bool b_force_enable );
static int MP4_Box_Init(demux_t *p_demux, MP4_Box_t *p_trak, int type);
...
static int MP4_TrackSeekByPos( demux_t *p_demux, mp4_track_t *p_track,
                          uint64_t i_pos );
...
/*add by jyq,get mp4 length*/
static mtime_t MP4_GetLength( demux_t *p_demux )
{
...
static bool MP4_stream_Tell_in( demux_t *p_demux, uint64_t *pi_pos )
{
    demux_sys_t *p_sys = p_demux->p_sys;
    if (p_sys->rawPage != NULL && !p_sys->b_SkipPage)
    {
        //msg_DbgVerbose(NULL, "MP4_stream_Tell_in play_stat rawPos=%lld %llu", p_sys->rawPos, p_sys->rawPos);
        *pi_pos = p_sys->rawPos;
        return true;
    }
    else
    {
        return MP4_stream_Tell(p_sys->p_stream, pi_pos);
    }
}

static int MP4_stream_Seek(demux_t *p_demux, uint64_t i_pos )
{
    demux_sys_t *p_sys = p_demux->p_sys;
    if (p_sys->rawPage != NULL && !p_sys->b_SkipPage)
    {
        msg_DbgVerbose(NULL, "MP4_stream_Seek i_pos=%llu p_sys->rawPage->end=%lld", i_pos, p_sys->rawPage->end);
        if (i_pos < p_sys->rawPage->end)
        {
            p_sys->rawPos = i_pos;
        ...
        {
            p_sys->b_SkipPage = true;
            return stream_Seek(p_sys->p_stream, i_pos);
        }
    }
    else
    {
...
static int LoadInitFrag( demux_t *p_demux )
{
    ...
    {
        if (p_sys->p_root != NULL)
        {
            if (MP4_BoxParse(p_sys->p_root) == NULL)
            {
                goto LoadInitFragError;
            }
        }
        else
        {
            ...
            MP4_Box_sys_t box_sys;
            box_sys.priv = p_demux;
            box_sys.fun = MP4_Box_Init;
            if( ( p_sys->p_root = MP4_BoxGetRoot( p_demux->s, &p_sys->root_preview, &box_sys) ) == NULL )
            {
                goto LoadInitFragError;
            }
        }
...
static int InitTracks( demux_t *p_demux )
{
    ...
    int i_tracks = 0;
    if (p_sys->hasInitedFirstTrack)
    {
        i_tracks = 1;
    }
    if (p_sys->hasInitedSecondTrack)
    {
        i_tracks = 2;
    }

    p_sys->track = realloc( p_sys->track, sizeof( mp4_track_t ) * p_sys->i_tracks);

    for(; i_tracks < p_sys->i_tracks; i_tracks++)
    {
        memset(&p_sys->track[i_tracks], 0, sizeof(mp4_track_t));
    }
...
static block_t * MP4_Block_Read( demux_t *p_demux, const mp4_track_t *p_track, int i_size )
{
    ...
    if (p_sys->rawPage != NULL && !p_sys->b_SkipPage)
    {
        if( i_size <= 0 )
        {
            msg_Dbg(p_demux, "mp4 block read failed.");
        ...
        if (p_bk != NULL)
        {
            Page *p = p_sys->rawPage;
            int i_read = 0;
            if (p_sys->rawPos >= p->pos && p_sys->rawPos < p->end)
            {
                int offset = p_sys->rawPos - p->pos;
                i_read = __MIN(i_size, p->end - p_sys->rawPos);
                memcpy(p_bk->p_buffer, p->data + offset, i_read);
                //p_bk->i_buffer += i_read;
                p_sys->rawPos += i_read;
            }

            msg_DbgVerbose(NULL, "MP4_Block_Read i_read=%d i_size=%d", i_read, i_size);
            if (i_read < i_size)
            {
                p_sys->b_SkipPage = true;

                int i_ret = stream_Read( p_sys->p_stream, p_bk->p_buffer + i_read, i_size - i_read);

                if( i_ret > 0 )
                {
                    p_bk->i_buffer = i_ret + i_read;
                    p_block = p_bk;
                }
                else
                {
    ...
    {
        p_block = stream_Block( p_sys->p_stream/*p_demux->s*/, i_size );
    }

    //block_t *p_block = stream_Block( p_sys->p_stream/*p_demux->s*/, i_size );
    if ( !p_block )
        return NULL;
    
    msg_DbgVerbose(NULL, "MP4_Block_Read p_block i_buffer=%d ", p_block->i_buffer);
...
{
    msg_DbgVerbose(NULL, "enter into mp4. play_stat_start");
    mtime_t open_start = mdate();
    ...
    vlc_value_t val;
    val.i_time = 1;
    if(var_GetTime( p_input, "mp4-demuxpos" )<=0)
      var_Create( p_input, "mp4-demuxpos", VLC_VAR_TIME );
    var_Change( p_input, "mp4-demuxpos", VLC_VAR_SETVALUE, &val, NULL );
    //add by zhgq end
    ...
    p_sys->hasInit = false;
    p_demux->pf_demux = Demux_distribute;
    p_sys->demuxType = DEMUX_TYPE_NORMAL;
    p_sys->pf_demux = Demux;
    p_sys->p_stream = p_demux->s;
    p_demux->pf_control = Control;
    p_sys->rawPage = NULL;
    p_sys->rawPos = 0;
    p_sys->error_state = false;
    p_sys->lastSamplePos = 0;
    p_sys->p_audio_block = NULL;

    p_sys->b_demux = false;

    p_demux->p_sys = p_sys;
    p_sys->last_seek_time = 0;
    //add by zhgq
    p_sys->i_time=0;
    //add by zhgq end

    vlc_mutex_init(&p_sys->demux_lock);
    vlc_cond_init(&p_sys->demux_cond);

    p_sys->parse_wait_tag = MP4_PARSE_LOCK_NORMAL;
    vlc_mutex_init(&p_sys->parse_lock);
    vlc_cond_init(&p_sys->parse_cond);

#ifdef MP4_IO_TEST_FP
    p_sys->fp = fopen("/sdcard/mp4_io_test3.log", "a+");
    if (p_sys->fp == NULL)
    {
        msg_DbgVerbose(NULL, "open mp4 io test fp failed. %s", strerror(errno));
    }
#endif
    ...
    if (vlc_clone(&p_sys->thread, mp4_thread, p_this, VLC_THREAD_PRIORITY_AUDIO))
    {
        return VLC_EGENERIC;
    }

    mtime_t open_cost = (mdate() - open_start) / 1000;
    var_SetTime(p_input, "probe_cost", open_cost);
...
static int mp4box_parse(demux_t *p_demux)
{
    msg_Dbg(p_demux, "play_stat_start mp4 mp4box_parse start");
    demux_sys_t *p_sys = p_demux->p_sys;
    if ( !p_sys->hasInit )
    {
        mtime_t open_start = mdate();
        p_sys->stream_cost = mdate();
        MP4_Box_t       *p_ftyp;
        MP4_Box_t       *p_rmra;
        MP4_Box_t       *p_mvhd;
        MP4_Box_t       *p_trak;
        ...
        mtime_t open_stream_start = mdate();
        ...
        msg_Dbg(p_demux, "play_stat_start mp4 mp4box_parse has new init.");

        if( p_sys->p_root &&
        p_sys->p_root->i_type == ATOM_root )
        {
            if (MP4_BoxGet( p_sys->p_root, "moov" ) == NULL)
            {
                if (p_sys->p_root->p_stream != NULL)
                {
                    msg_Dbg(p_demux, "play_stat_start mp4box_parse another stream. %s", p_sys->p_root->new_stream_url);
                    if( LoadInitFrag( p_demux ) != VLC_SUCCESS )
                    {
                        goto error;
                    }
        ...
        msg_Dbg(p_demux, "play_stat_start mp4box_parse init frag box %"PRId64, (mdate() - open_stream_start));

        mtime_t open_stream_cost = (mdate() - open_stream_start) / 1000;
        //var_SetTime ( p_input, "dl_time", open_stream_cost );
        ...
        if ( p_sys->b_smooth )
        {
            p_sys->demuxType = DEMUX_TYPE_FRAGMENT;
            p_sys->pf_demux = DemuxFrg;
        }
        else if( p_sys->b_fragmented )
        {
            p_sys->demuxType = DEMUX_TYPE_ASLEAF;
            p_sys->pf_demux = DemuxAsLeaf;
        }
        ...
        if( !(p_mvhd = MP4_BoxGet( p_sys->p_root, "/moov/mvhd" ) ) )
        {
            ...
            else
            {
                ...
                p_sys->demuxType = DEMUX_TYPE_REF;
                p_sys->pf_demux = DemuxRef;
                goto success;
            }
        }
        else
        ...
        msg_Dbg(p_demux, "play_stat open_mp4 i_tracks=%u i_overall_duration=%llu b_fragmented=%d",
         p_sys->i_tracks, p_sys->i_overall_duration, p_sys->b_fragmented);
        ...
        msg_Dbg( p_demux, "play_stat found %d track%c",
                            p_sys->i_tracks,
                            p_sys->i_tracks ? 's':' ' );
        ...
        b_enabled_es = false;
        for( i = p_sys->hasInitedFirstTrack ? 1 : 0; i < p_sys->i_tracks; i++ )
        {
        ...
        for( i = p_sys->hasInitedFirstTrack ? 1 : 0; i < p_sys->i_tracks; i++ )
        {
            ...
            msg_Dbg(p_demux, "mp4_box_parse play_stat p_trak=%p", p_trak);
            ...
            if( p_sys->track[i].b_ok && !p_sys->track[i].b_chapter )
            {
                ...
                msg_Dbg( p_demux, "play_stat adding track[Id 0x%x] %s (%s) language %s",
            ...
            {
                msg_Dbg( p_demux, "play_stat using track[Id 0x%x] for chapter language %s",
                         p_sys->track[i].i_track_ID,
                         p_sys->track[i].fmt.psz_language ?
                         p_sys->track[i].fmt.psz_language : "undef" );
            }
            ...
            {
                msg_Dbg( p_demux, "play_stat ignoring track[Id 0x%x]",
                         p_sys->track[i].i_track_ID );
            }
        }
        ...
        while ( p_fragment && p_sys->i_tracks )
        {
            ...
            msg_Dbg( p_demux, "play_stat fragment offset %"PRId64", data %"PRIu64"<->%"PRIu64", "
            ...
            msg_Dbg( p_demux, "jyq test play_stat mp4 moov size is %"PRId64, p_fragment->i_chunk_range_min_offset);
            var_SetTime(p_input, "dl_size", p_fragment->i_chunk_range_min_offset);
        ...
        mtime_t open_cost = (mdate() - open_start) / 1000;
        //var_SetTime(p_input, "probe_cost", 0);
        msg_Dbg(p_demux, "play_stat_start: test mp4 open cost is %"PRId64 ",open stream cost %"PRId64, open_cost,open_stream_cost);
...
static void parse_wait(demux_t *p_demux, int64_t timecost)
{
    demux_sys_t *p_sys = p_demux->p_sys;

    vlc_mutex_lock(&p_sys->parse_lock);

    if (timecost == -1)
    {
        if (p_sys->parse_wait_tag != MP4_PARSE_LOCK_SIGNAL)
        {
            p_sys->parse_wait_tag = MP4_PARSE_LOCK_WAIT;

            vlc_cond_wait(&p_sys->parse_cond, &p_sys->parse_lock);
        }
        else
        {
            vlc_cond_timedwait(&p_sys->parse_cond, &p_sys->parse_lock, mdate()+10000);
        }
    }
    else
    {
        vlc_cond_timedwait(&p_sys->parse_cond, &p_sys->parse_lock, mdate()+timecost);
    }

    vlc_mutex_unlock(&p_sys->parse_lock);
}

static void parse_signal(demux_t *p_demux)
{
    demux_sys_t *p_sys = p_demux->p_sys;

    vlc_mutex_lock(&p_sys->parse_lock);
    p_sys->parse_wait_tag = MP4_PARSE_LOCK_SIGNAL;
    vlc_cond_signal(&p_sys->parse_cond);
    vlc_mutex_unlock(&p_sys->parse_lock);
}

static void demux_wait(demux_t *p_demux, int64_t timecost)
{
    demux_sys_t *p_sys = p_demux->p_sys;

    vlc_mutex_lock(&p_sys->demux_lock);
    while (!p_sys->hasInit && !p_sys->error_state && !OPTIMIZED_TYPE)
    {
        if (timecost == -1)
        {
            vlc_cond_wait(&p_sys->demux_cond, &p_sys->demux_lock);
        }
        else
        {
            vlc_cond_timedwait(&p_sys->demux_cond, &p_sys->demux_lock, mdate()+timecost);
        }
    }
    vlc_mutex_unlock(&p_sys->demux_lock);
}

static void demux_signal(demux_t *p_demux)
{
    demux_sys_t *p_sys = p_demux->p_sys;

    vlc_mutex_lock(&p_sys->demux_lock);
    vlc_cond_signal(&p_sys->demux_cond);
    vlc_mutex_unlock(&p_sys->demux_lock);
}
...
static void *mp4_thread(void *p_this)
{
    demux_t  *p_demux = (demux_t *)p_this;
    demux_sys_t *p_sys = p_demux->p_sys;
    if (mp4box_parse(p_demux) != VLC_SUCCESS)
    {
        p_sys->error_state = true;
    }

    MP4_Box_t *root = p_sys->root_preview;
    if (root != NULL && root->boxType == MP4_BOX_CUSTOME_TYPE_NORMAL && root->stream_set_flag)
    {
        if (root->moovBoxType == MP4_BOX_MOOV_POS_FRONT)
        {
            msg_DbgVerbose(NULL, "_stream_ url close front.");

            stream_Control(p_demux->s, STREAM_ACCESS_CLOSE);
        }
        else if (root->moovBoxType == MP4_BOX_MOOV_POS_END)//a
        {
            msg_DbgVerbose(NULL, "_stream_ url close end");

            stream_Control(root->p_stream, STREAM_ACCESS_CLOSE);
        }
    }

    parse_signal(p_demux);
    demux_signal(p_demux);

    return NULL;
}

static bool initRawPage(demux_t *p_demux)
{
    demux_sys_t *p_sys = p_demux->p_sys;

    if (!p_sys->hasInit && p_sys->rawPage == NULL && p_sys->pageSize > 0)
    {
        int iRead = 0;
        Page *rawPage = malloc(sizeof(Page));
        memset(rawPage, 0, sizeof (rawPage));
        rawPage->data = malloc(p_sys->pageSize + 32);
        p_sys->rawPage = rawPage;
        int64_t cost = mdate();
        if (rawPage != NULL && rawPage->data != NULL)
        {
            p_sys->b_SkipPage = false;
            rawPage->pos = rawPage->end = p_sys->rawPos = stream_Tell( p_sys->p_stream );
            p_sys->lastSamplePos = rawPage->pos;

            int ret = 0;
             while (!p_sys->hasInit
                && !OPTIMIZED_TYPE
                && iRead < p_sys->pageSize
                && !p_sys->error_state
                && ((ret = stream_Read( p_sys->p_stream, rawPage->data + iRead, __MIN(STREAM_READ_ATONCE, p_sys->pageSize - iRead))) > 0))
            {
                iRead += ret;
                rawPage->end += ret;
            }

            msg_DbgVerbose(NULL, "play_stat_start: another stream read mdata. stream_new_cost=%lld iRead=%ld cost=%lld speed=%ldKB/s ",
                p_sys->root_preview->stream_new_cost, iRead, (mdate() - cost), (iRead * 1000 / (mdate() - cost + 1)));

#ifdef MP4_IO_TEST_FP
            if (p_sys->fp != NULL)
            {
                fprintf(p_sys->fp, "%lld %ld %lld %ld \n",
                    p_sys->root_preview->stream_new_cost, iRead, (mdate() - cost), (iRead * 1000 / (mdate() - cost + 1)));
                fflush(p_sys->fp);
            }
#endif

            demux_wait(p_demux, -1);
        }
    }
}

static int Demux_distribute( demux_t *p_demux )
{
    demux_sys_t *p_sys = p_demux->p_sys;
    ...
    mtime_t demux_cost = 0;
    if (!p_sys->b_demux)
    {
        demux_cost = mdate();
        msg_DbgVerbose(NULL, "play_stat enter into distributed");
    }

    while (!p_sys->hasInit && !p_sys->error_state && !OPTIMIZED_TYPE)
    {
        parse_wait(p_demux, -1);
        ...
        if (root != NULL && root->boxType == MP4_BOX_CUSTOME_TYPE_NORMAL && root->stream_set_flag)
        {
            ...
            {
                int64_t cost = mdate();
                root->p_stream = stream_UrlNew(p_demux->s, root->new_stream_url);
                if (root->p_stream == NULL)
                {
                    p_sys->error_state = true;

                    msg_Err(p_demux, "_stream_ url new fail.");
                    return VLC_EGENERIC;
                }

                root->stream_new_cost = mdate() - cost;

                p_sys->p_stream = root->p_stream;
                free(root->new_stream_url);
                root->new_stream_url = NULL;

                int ret = MP4_FindBoxPos(p_sys->p_stream, ATOM_mdat);
                p_sys->lastSamplePos = stream_Tell( p_sys->p_stream );

                msg_DbgVerbose(NULL, "play_stat_start Demux_distribute front .has other stream. begin p_sys->lastSamplePos=%lld", p_sys->lastSamplePos);

                p_sys->pageSize = PAGE_SIZE;
                initRawPage(p_demux);

                msg_DbgVerbose(NULL, "play_stat_start: Demux_distribute front. has other stream. pos=%lld stream_new_cost=%lld", p_sys->rawPos, root->stream_new_cost);
            ...
            else if (root->moovBoxType == MP4_BOX_MOOV_POS_END)//a
            {
                ...
                MP4_FindBoxPos(p_sys->p_stream, ATOM_mdat);
                msg_DbgVerbose(NULL, "play_stat_start Demux_distribute end .has other stream. begin");

                p_sys->pageSize = PAGE_SIZE * 0.8;
                initRawPage(p_demux);

                msg_DbgVerbose(NULL, "play_stat_start Demux_distribute end .has other stream. pos=%lld", p_sys->rawPos);
    ...
    if (p_sys->error_state)
    {
        msg_DbgVerbose(NULL, "demux mp4 box parse error.");
    ...
    {
        input_thread_t *p_input = demux_GetParentInput( p_demux );
        var_SetTime ( p_input, "dl_time", mdate() - demux_cost);

        es_out_t *demux_out = p_demux->out;
        demux_out->demux_cost += mdate() - demux_cost;
        demux_out->demux_stream_cost += mdate() - demux_cost;

        msg_DbgVerbose(NULL, "Demux_distribute play_stat dl_time=%lld", mdate() - demux_cost);

        p_sys->b_demux = true;
    }

    return p_sys->pf_demux(p_demux);
}

static int MP4_SendData(demux_t *p_demux, mp4_track_t *tk,
    bool hasInit, bool hasOnlyInitedFirstTrack, bool m_trakunselected, uint64_t i_candidate_pos,
    bool *b_data_sent, uint32_t *i_samplessize, uint32_t *i_nb_samples)
{
    demux_sys_t *p_sys = p_demux->p_sys;
    es_out_t *demux_out = p_demux->out;

    mtime_t demux_stream_start;
    mtime_t demux_stream_cost;

    unsigned int i_track;
    int i_tracks = p_sys->i_tracks;
    if (!hasInit && OPTIMIZED_TYPE)
    {
        i_tracks = 1;
    }
    ...
    *i_samplessize = MP4_TrackGetReadSize( tk, i_nb_samples );
    if( *i_samplessize > 0 )
    {
        ...
        /* go,go go ! */
        if ( !MP4_stream_Tell_in( p_demux/*p_demux->s*/, &i_current_pos ) )
            goto end;

    msg_DbgVerbose(NULL, "MP4_SendData m_trakunselected=%d p_sys->b_seekmode=%d i_current_pos=%llu i_candidate_pos=%llu",
     m_trakunselected, p_sys->b_seekmode, i_current_pos, i_candidate_pos);
    //add by zhgq
    if(m_trakunselected == true && p_sys->b_seekmode == false)
    {
       if(i_candidate_pos > i_current_pos && (i_candidate_pos-i_current_pos)<1000000)
       {
       MP4_Block_Read( p_demux, tk, (i_candidate_pos-i_current_pos) );
       if ( !MP4_stream_Tell_in( p_demux/*p_demux->s*/, &i_current_pos ) )
            goto end;
       }
    }   

    msg_DbgVerbose(NULL, "MP4_SendData *i_samplessize=%lu i_current_pos=%llu, \
            i_candidate_pos=%llu hasInitedFirstTrack=%d hasInit=%d \
            lastSamplePos=%lld hasOnlyInitedFirstTrack=%d",
            *i_samplessize, i_current_pos, i_candidate_pos, p_sys->hasInitedFirstTrack,
            hasInit, p_sys->lastSamplePos, hasOnlyInitedFirstTrack);

    if (!hasInit && !p_sys->isSeek)
    {
        if (p_sys->lastSamplePos < i_candidate_pos)
        {
            demux_stream_start = mdate();
            if( MP4_stream_Seek( p_demux/*p_demux->s*/, p_sys->lastSamplePos )  )
            {
                msg_Warn( p_demux, "play_stat track[0x%x] will be disabled (eof?)"
                          ": Failed to seek to %"PRIu64,
                          tk->i_track_ID, p_sys->lastSamplePos);
                MP4_TrackUnselect( p_demux, tk );
                return VLC_EGENERIC;
            }

            mp4_track_t *audio_tk = NULL;
            if (hasOnlyInitedFirstTrack)
            {
                audio_tk = &p_sys->track[1];
            }

            block_t *p_next = MP4_Block_Read( p_demux, audio_tk, i_candidate_pos - p_sys->lastSamplePos );
            if (p_next != NULL)
            {
                if (p_sys->p_audio_block == NULL)
                {
                    p_sys->p_audio_block = p_next;
                }
                else
                {
                    p_sys->p_audio_block->p_next = p_next;
                }

                demux_stream_cost = (mdate() - demux_stream_start);
                demux_out->demux_stream_cost += demux_stream_cost;

                //dumpMemory(p_next->p_buffer, p_next->i_buffer, "dumpMemory demux p_next audio.");
            }
        }
    }
    ...
    //add by zhgq end
        ...
        {
            demux_stream_start = mdate();
            if( MP4_stream_Seek( p_demux/*p_demux->s*/, i_candidate_pos )  )
            {
                msg_Warn( p_demux, "play_stat track[0x%x] will be disabled (eof?)"
                          ": Failed to seek to %"PRIu64,
    ...
    else
    {
       ...
       //add zhgq end
        ...
        if (!hasInit)
        {
            ...
            msg_DbgVerbose(NULL, "MP4_stream_Tell_in lastSamplePos i_current_pos=%lld", i_current_pos);
            p_sys->lastSamplePos = i_current_pos;
        }
        ...
        {
            //dumpMemory(p_block->p_buffer, p_block->i_buffer, "dumpMemory demux video.");
        }

        msg_DbgVerbose(NULL, "Demux i_cat=%d dts=%lld", tk->fmt.i_cat, p_block->i_dts);
        if( MP4_TrackGetPTSDelta( p_demux, tk, &i_delta ) )
    {
        ...
        else if( tk->fmt.i_cat != VIDEO_ES )
            {
                ...
                //msg_Dbg(p_demux,"jyq test audio pts = %"PRId64, p_block->i_pts);
            }
        else
    ...
    {
        p_block->i_pts=p_block->i_dts;
        //msg_Dbg(p_demux,"jyq test invalid pts = %"PRId64, p_block->i_pts);
    }   
        msg_DbgVerbose(NULL, "Demux i_cat=%d. block pts=%lld dts=%lld p_sys->isSeek=%d",
         tk->fmt.i_cat, p_block->i_pts, p_block->i_dts, p_sys->isSeek);
    //add by zhgq end
        if ( !(*b_data_sent) )
        {
            mtime_t pcr = VLC_TS_0 + p_sys->i_pcr;
            if (p_sys->isSeek && p_sys->f_fps > 1.0f )
            {
                pcr = p_block->i_pts + 1 / p_sys->f_fps * CLOCK_FREQ;
                p_sys->isSeek = false;
                es_out_Control( p_demux->out, ES_OUT_SET_NEXT_DISPLAY_TIME, pcr );
            }
            es_out_Control( p_demux->out, ES_OUT_SET_PCR, pcr );

            //msg_Dbg(p_demux,"jyq test pcr = %"PRId64, p_sys->i_pcr);
            
            *b_data_sent = true;
        }

        ssize_t expectSize = -1;
        if (tk->fmt.i_cat == VIDEO_ES)
        {
            ...
            {
                if( tk->i_sample_size == 0 ) /* all sizes are different */
                {
                    expectSize = tk->p_sample_size[tk->i_sample + 1];
                }
                else
                {
                    expectSize = tk->i_sample_size;
                }
            }
        }
        p_block->expectSize = expectSize;
        //dumpMemoryIO(p_block->p_buffer, p_block->i_buffer, "play_stat_start: dumpMemory demux p_block.", "%d", tk->fmt.i_cat);
        MP4_Block_Send( p_demux, tk, p_block );
        if (p_sys->p_audio_block != NULL && p_sys->hasInitedSecondTrack)
        {
            p_sys->p_audio_block->i_pts = p_sys->p_audio_block->i_dts = VLC_TS_0 + p_sys->i_pcr;
            MP4_Block_Send( p_demux, &p_sys->track[1], p_sys->p_audio_block );
            p_sys->p_audio_block = NULL;
        }
    }
    /* Next sample */
    ...
    {
    //add by zhgq 
    mtime_t audio_es_pcr=0,video_es_pcr=0;
    int i_selectedtracks=0;
    //add by zhgq end
        p_sys->i_pcr = INT64_MAX;
        for( i_track = 0; i_track < i_tracks; i_track++ )
        {
        ...
        if(tk->fmt.i_cat == AUDIO_ES)
        audio_es_pcr=__MIN( i_dts, p_sys->i_pcr );
        
        else if(tk->fmt.i_cat == VIDEO_ES)
        video_es_pcr=__MIN( i_dts, p_sys->i_pcr );
        }
    //msg_Dbg( p_demux, "MainLoopInterface   (audio=%"PRId64 "video=%"PRId64 "i_pcr=%"PRId64"internal = %"PRId64,audio_es_pcr,video_es_pcr,p_sys->i_pcr ,audio_es_pcr-video_es_pcr);
      
      if(i_selectedtracks >=2)
      {
        if(audio_es_pcr==0 ||  video_es_pcr ==0)
          p_sys->i_moviedelta=__MAX( audio_es_pcr, video_es_pcr ) - __MIN( audio_es_pcr, video_es_pcr );
        p_sys->i_pcr = __MIN( audio_es_pcr, video_es_pcr );
      }
      else
        p_sys->i_pcr = __MAX( audio_es_pcr, video_es_pcr );

        p_sys->i_pcr = __MAX(p_sys->i_pcr, 1);
        msg_DbgVerbose(NULL, "MainLoopInterface p_sys->i_pcr=%lld", p_sys->i_pcr);
      if( p_sys->i_pcr>CLOCK_FREQ && i_selectedtracks >=2 )
       {
        if((audio_es_pcr - video_es_pcr)  < -( p_sys->i_moviedelta + 1*CLOCK_FREQ))
         p_sys->i_pcr = __MAX( audio_es_pcr, video_es_pcr );
       }
        
        //add by zhgq end
        //change by zhgq begin
            /* delete this
...
static int Demux( demux_t *p_demux )
{
    ...
    unsigned int i_track_selected = 0;
    mtime_t demux_stream_start=0;
    /* check for newly selected/unselected track */
    int i_tracks = p_sys->i_tracks;
    bool hasOnlyInitedFirstTrack = false;
    bool hasInit = p_sys->hasInit;
    if (!hasInit && OPTIMIZED_TYPE)
    {
        i_tracks = 1;
    }
    if (!hasInit && OPTIMIZED_TYPE && p_sys->hasInitedSecondTrack)
    {
        hasOnlyInitedFirstTrack = true;
    }

    for( i_track = 0, i_track_selected = 0; i_track < i_tracks;
         i_track++ )
    {
        mp4_track_t *tk = &p_sys->track[i_track];
        bool b;
    ...
    msg_DbgVerbose(NULL, "demux i_track_selected=%lu", i_track_selected);
    ...
    bool m_trakunselected=false;
    for( i_track = 0; i_track < i_tracks; i_track++ )
    {
        mp4_track_t *tk_tmp = &p_sys->track[i_track];
	if(!tk_tmp->b_selected)
		m_trakunselected=true;
        if( !tk_tmp->b_ok || tk_tmp->b_chapter || !tk_tmp->b_selected || tk_tmp->i_sample >= tk_tmp->i_sample_count )
        {
            msg_DbgVerbose(NULL, "b_ok=%d b_chapter=%d b_selected=%d i_sample=%u i_sample_count=%u", tk_tmp->b_ok, tk_tmp->b_chapter, tk_tmp->b_selected, tk_tmp->i_sample, tk_tmp->i_sample_count);
            continue;
        }
        ...
        {
            if (hasInit && p_sys->lastSamplePos > 0 && tk_tmp->fmt.i_cat == AUDIO_ES)
            {
                MP4_TrackSeekByPos(p_demux, tk_tmp, p_sys->lastSamplePos);
            }
            /* Try to avoid seeking on non fastseekable. Will fail with non interleaved content */
            uint64_t i_pos = MP4_TrackGetPos( tk_tmp );
            msg_DbgVerbose(NULL, "Demux i_cat=%d i_pos=%lld lastSamplePos=%lld", tk_tmp->fmt.i_cat, i_pos, p_sys->lastSamplePos);

            if (hasInit && p_sys->lastSamplePos > 0)
            {
                if (i_pos >= p_sys->lastSamplePos)
                {
    ...
    MP4_SendData(p_demux, tk, hasInit, hasOnlyInitedFirstTrack, m_trakunselected, i_candidate_pos, &b_data_sent, &i_samplessize, &i_nb_samples);

    mtime_t demux_cost = mdate() - demux_start;
    demux_out->demux_cost += demux_cost;
...
static int Seek( demux_t *p_demux, mtime_t i_date )
{
    ...
    p_sys->last_seek_time = i_date;
    p_sys->i_pcr  = VLC_TS_INVALID;
    int video_seek_tk_id = -1;
    int audio_seek_tk_id = -1;

    msg_DbgVerbose(NULL, "mp4_seek i_date=%lld i_tracks=%d hasInit=%d hasInitedFirstTrack=%d hasInitedSecondTrack=%d",
     i_date, p_sys->i_tracks, p_sys->hasInit, p_sys->hasInitedFirstTrack, p_sys->hasInitedSecondTrack);
    ...
    for( i_track = 0; i_track < i_tracks; i_track++ )
    {
        ...
        if (tk->fmt.i_cat == VIDEO_ES)
        {
            ...
            video_seek_tk_id = i_track;
        }
    }
    
    // modify by tanzhihua. seek to audio.
    if (video_seek_tk_id != -1 && MP4_GetLength( p_demux ) != i_date)
    {
        mp4_track_t *video_tk = &p_sys->track[video_seek_tk_id];
        mtime_t seek_date = MP4_TrackGetDTS( p_demux, video_tk );
        int64_t video_pos = MP4_TrackGetPos( video_tk );

        msg_DbgVerbose(NULL, "MP4_TrackSeek seek video_seek_tk_id=%d CLOCK_FREQ=%lld i_date=%lld i_time=%lld last_seek_time=%lld i_timescale=%lu seek_date=%lld i_overall_duration=%lld video_pos=%lld",
         video_seek_tk_id, CLOCK_FREQ, i_date, p_sys->i_time, p_sys->last_seek_time, p_sys->i_timescale, seek_date, p_sys->i_overall_duration, video_pos);

        for( i_track = 0; i_track < i_tracks; i_track++ )
        {
            mp4_track_t *tk = &p_sys->track[i_track];
            if (tk->fmt.i_cat != VIDEO_ES && tk->b_selected)
            {
                MP4_TrackSeek( p_demux, tk, seek_date );
                int64_t candidate_pos = MP4_TrackGetPos( tk );
                if (video_pos - candidate_pos > 50000)
                {
                    msg_DbgVerbose(NULL, "MP4_TrackSeek refer video, abs(candidate_pos - video_pos)=%lld", abs(candidate_pos - video_pos));
                    MP4_TrackSeek( p_demux, tk, i_date );
                }
            }
        }
    ...
    if ( MP4_GetLength( p_demux ) > 80 * CLOCK_FREQ  && MP4_GetLength( p_demux ) != i_date)
    {
        p_sys->isSeek = true;
    }
...
static int Control( demux_t *p_demux, int i_query, va_list args )
{
    ...
    switch( i_query )
    {
        ...
        case DEMUX_SET_POSITION:
            ...
            if ( p_sys->demuxType == DEMUX_TYPE_ASLEAF /*p_demux->pf_demux == DemuxAsLeaf */)
                return LeafSeekToPos( p_demux, f );
            else if ( p_sys->demuxType == DEMUX_TYPE_FRAGMENT /*p_demux->pf_demux == DemuxFrg */)
        ...
        case DEMUX_SET_TIME:
            ...
            msg_DbgVerbose(NULL, "DEMUX_SET_TIME i64=%lld", i64);
            if ( p_sys->demuxType == DEMUX_TYPE_ASLEAF /*p_demux->pf_demux == DemuxAsLeaf*/ )
        ...
        case DEMUX_GET_PTS_DELAY:
        {
            ...
            for( unsigned int i = 0; i < p_sys->i_tracks; i++ )
            {
                ...
                if ( p_sys->track != NULL && (p_load = MP4_BoxGet( p_sys->track[i].p_track, "load" )) &&
        ...
        case DEMUX_CAN_RECORD:
            return VLC_EGENERIC;
        case DEMUX_STATE_RESET:
            msg_DbgVerbose(NULL, "DEMUX_STATE_RESET");
            parse_signal(p_demux);
            demux_signal(p_demux);
            return VLC_SUCCESS;
        default:
...
{
    msg_DbgVerbose(NULL, "enter into mp4 close");

    demux_t *  p_demux = (demux_t *)p_this;
    demux_sys_t *p_sys = p_demux->p_sys;
    unsigned int i_track;

    p_sys->error_state = true;//exit

    vlc_join(p_sys->thread, NULL);

    vlc_mutex_destroy(&p_sys->demux_lock);
    vlc_cond_destroy(&p_sys->demux_cond);
    vlc_mutex_destroy(&p_sys->parse_lock);
    vlc_cond_destroy(&p_sys->parse_cond);

    //add by zhgq
    input_thread_t *p_input = demux_GetParentInput( p_demux );
    var_Destroy(p_input, "video_dl_size");
    //add by zhgq
    if (p_sys->p_root->new_stream_url != NULL)
    {
        free(p_sys->p_root->new_stream_url);
    }
    if (p_sys->p_root->p_stream != NULL)
    {
        stream_Delete(p_sys->p_root->p_stream);
    }

    block_t *block = p_sys->p_audio_block;
    while (block != NULL)
    {
        block_t *buf;
        buf = block->p_next;
                                    ...
                                    mp4_track_t *p_demux_track )
{
    ...
    else
    {
        ...
        msg_Dbg( p_demux, "STTS table of %"PRIu32" entries", stts->i_entry_count );
        ...
        for( uint32_t i_chunk = 0; i_chunk < p_demux_track->i_chunk_count; i_chunk++ )
        {
			...
			// msg_DbgVerbose(NULL, "TrackCreateSamplesIndex i_chunk=%d i_entry=%lu i_index=%d i_current_index_samples_left=%lu i_sample_count=%lu "
			// 	, i_chunk, i_entry, i_index, i_current_index_samples_left, i_sample_count);
...
{
    msg_DbgVerbose(NULL, "enter into TrackCreateES i_chunk=%u", i_chunk);
                                   ...
                                   uint32_t *pi_sample )
{
    ...
    if( p_track->p_elst && p_track->BOXDATA(p_elst)->i_entry_count > 0 )
    {
        ...
        msg_Dbg( p_demux, "elst (%d) gives %"PRId64"ms (movie)-> %"PRId64
                 "ms (track) i_elst_time=%lld timescale=%lu", p_track->i_elst,
                 i_mvt * 1000 / p_sys->i_timescale,
                 i_start * 1000 / p_track->i_timescale, p_track->i_elst_time, p_track->i_timescale);
    }
    else
    {
	...
	msg_DbgVerbose(NULL, "TrackTimeToSampleChunk i_chunk=%d i_offset=%llu i_sample_description_index=%lu i_sample_count=%lu i_sample_first=%lu i_sample=%lu i_first_dts=%lu",
        i_chunk,
	    p_track->chunk[i_chunk].i_offset,
        p_track->chunk[i_chunk].i_sample_description_index,
        p_track->chunk[i_chunk].i_sample_count,
        p_track->chunk[i_chunk].i_sample_first,
        p_track->chunk[i_chunk].i_sample,
        p_track->chunk[i_chunk].i_first_dts);
    /* *** find sample in the chunk *** */
    i_sample = p_track->chunk[i_chunk].i_sample_first;
    i_dts    = p_track->chunk[i_chunk].i_first_dts;

    msg_DbgVerbose(NULL, "TrackTimeToSampleChunk i_chunk=%d p_sample_count_dts=%d p_sample_delta_dts=%d i_stts_entry_count=%u",
         i_chunk, p_track->chunk[i_chunk].p_sample_count_dts[0],
         p_track->chunk[i_chunk].p_sample_delta_dts[0],
         p_track->chunk[i_chunk].i_stts_entry_count);

    for( i_index = 0; i_index < p_track->chunk[i_chunk].i_stts_entry_count; )
    {
        if( i_dts +
    ...
    if( ( p_box_stss = MP4_BoxGet( p_track->p_stbl, "stss" ) ) )
    {
        ...
        msg_Dbg( p_demux, "track[Id 0x%x] using Sync Sample Box (stss). count=%d",
                 p_track->i_track_ID, p_stss->i_entry_count);
...
// only support no more than two tracks; first track must video, second track must audio.
static int MP4_Box_AddTrack(demux_t *p_demux, MP4_Box_t *p_trak, int type)
{
    demux_sys_t *p_sys = p_demux->p_sys;

    if (!p_sys->b_optimaized)
    {
        return VLC_EGENERIC;
    }

    if ((p_sys->hasInitedFirstTrack && type == VIDEO_ES)
        || (p_sys->hasInitedSecondTrack && type == AUDIO_ES)
        || (type == AUDIO_ES && !p_sys->hasInitedFirstTrack))
    {
        return VLC_EGENERIC;
    }

    int i_tracks = 0;
    if (type == VIDEO_ES && !p_sys->hasInitedFirstTrack)
    {
        i_tracks = 1;
    }
    if (type == AUDIO_ES && !p_sys->hasInitedSecondTrack)
    {
        i_tracks = 2;
    }

    p_sys->track = realloc( p_sys->track, sizeof( mp4_track_t ) * 2);
    
    msg_DbgVerbose(NULL, "MP4_Box_AddTrack play_stat add track. i_tracks=%d %p %p %p", i_tracks, p_trak, &p_sys->track[0], &p_sys->track[1]);
    ...
    {
        msg_DbgVerbose(NULL, "play_stat trak do no thas tkhd box");
        return VLC_EGENERIC;
    }

    MP4_Box_t *p_chap = MP4_BoxGet( p_trak, "tref/chap", 0 );//first video track
    if( p_chap && p_chap->data.p_tref_generic &&
        p_chap->data.p_tref_generic->i_entry_count > 0 && !p_sys->p_tref_chap )
    {
        p_sys->p_tref_chap = p_chap;
    ...
    {
        msg_DbgVerbose(NULL, "play_stat trak do no thas tref/chap box");
    }

    if (type == VIDEO_ES)
    {
        memset(&p_sys->track[0], 0, sizeof(mp4_track_t));
        MP4_TrackCreate(p_demux, &p_sys->track[0], p_trak, b_enabled_es);
        if (p_sys->track[0].fmt.i_cat == VIDEO_ES)
        {
            p_sys->hasInitedFirstTrack = true;
            demux_signal(p_demux);

            p_sys->stream_cost = mdate() - p_sys->stream_cost;

            msg_DbgVerbose(NULL, "play_stat_start: parse video trak. stream_cost=%lld stream_read_size=%lld stream_read_cost=%lld speed=%lldKB stream_seek_cost=%lld \
                stream_peek_cost=%lld stream_parse_cost=%lld",
            p_sys->stream_cost,
            p_sys->root_preview->stream_read_size,
            p_sys->root_preview->stream_read_cost,
            p_sys->root_preview->stream_read_size * 1000 / (p_sys->root_preview->stream_read_cost + 1),
            p_sys->root_preview->stream_seek_cost, p_sys->root_preview->stream_peek_cost,
            p_sys->root_preview->stream_parse_cost);

#ifdef MP4_IO_TEST_FP
            if (p_sys->fp != NULL)
            {
                fprintf(p_sys->fp, "%lld %lld %lld %lld %lld\n",
                    p_sys->stream_cost,
                    p_sys->root_preview->stream_parse_cost,
                    p_sys->root_preview->stream_read_size,
                    p_sys->root_preview->stream_read_cost,
    ...
    {
        memset(&p_sys->track[1], 0, sizeof(mp4_track_t));
        if (MP4_AudioTrackPartInit(p_demux, &p_sys->track[1], p_trak, b_enabled_es) != VLC_EGENERIC)
        {
            p_sys->hasInitedSecondTrack = true;
        }
    }

    if( p_sys->track[0].b_ok && !p_sys->track[0].b_chapter )
    {
        const char *psz_cat;
        switch( p_sys->track[0].fmt.i_cat )
        {
            case( VIDEO_ES ):
                psz_cat = "video";
                break;
        ...
        msg_Dbg( p_demux, "adding track[Id 0 0x%x] %s (%s) language %s",
                 p_sys->track[0].i_track_ID, psz_cat,
                 p_sys->track[0].b_enable ? "enable":"disable",
                 p_sys->track[0].fmt.psz_language ?
                 p_sys->track[0].fmt.psz_language : "undef" );
    }
    else if( p_sys->track[0].b_ok && p_sys->track[0].b_chapter )
    {
        msg_Dbg( p_demux, "using track[Id=0 0x%x] for chapter language %s",
                 p_sys->track[0].i_track_ID,
                 p_sys->track[0].fmt.psz_language ?
                 p_sys->track[0].fmt.psz_language : "undef" );
    }
    else
    {
        msg_Dbg( p_demux, "ignoring track[Id=0 0x%x]",
                 p_sys->track[0].i_track_ID );
    }
...
static int MP4_Box_Init(demux_t *p_demux, MP4_Box_t *p_trak, int type)
{
    ...
    msg_DbgVerbose(NULL, "MP4_Box_Init play_stat_start: type=%d", type);

    if (!p_sys->hasInitedFirstTrack && type == MP4_BOX_INIT_TYPE_INIT_MP4A_STSD)
    {
        p_sys->b_optimaized = false;
    }

    switch (type)
    {
        case MP4_BOX_INIT_TYPE_ADD_TRACK:
            return MP4_Box_AddTrack(p_demux, p_trak, VIDEO_ES);
        case MP4_BOX_INIT_TYPE_INIT_MVHD:
            {
                MP4_Box_t *p_mvhd = p_trak;
                p_sys->i_timescale = BOXDATA(p_mvhd)->i_timescale;
                p_sys->i_overall_duration = BOXDATA(p_mvhd)->i_duration;
                msg_DbgVerbose(NULL, "mp4 box init play_stat mvhd i_timescale=%lu i_next_track_id=%d i_duration=%llu i_overall_duration=%llu",
                 p_sys->i_timescale, p_mvhd->data.p_mvhd->i_next_track_id, p_mvhd->data.p_mvhd->i_duration, p_sys->i_overall_duration);
                if (p_mvhd->data.p_mvhd->i_next_track_id == 3)
                {
                    p_sys->b_optimaized = true;
                }
                if( p_sys->i_timescale == 0 )
                {
                    msg_DbgVerbose( p_demux, "bad timescale" );
                    return VLC_EGENERIC;
                }
            ...
            break;
        case MP4_BOX_INIT_TYPE_INIT_MP4A_STSD:
            return MP4_Box_AddTrack(p_demux, p_trak, AUDIO_ES);
        case MP4_BOX_INIT_TYPE_DATA:
            parse_signal(p_demux);
...
static void MP4_TrackCreate( demux_t *p_demux, mp4_track_t *p_track,
                             ...
                             bool b_force_enable )
{
    msg_DbgVerbose(NULL, "enter into play_stat %s.", __FUNCTION__);
...
/*        msg_Warn( p_demux, "unhandled box: tref --> FIXME" ); */
    ...
    if( ( !p_mdhd )||( !p_hdlr ) )
    {
        msg_DbgVerbose(NULL, "MP4_TrackCreate do not has mdhd box");
        return;
    }
...
#define MAX_SELECTABLE (INT_MAX - ES_PRIORITY_SELECTABLE_MIN)
    ...
    msg_DbgVerbose(NULL, "MP4_TrackCreate p_track->i_chunk=%d i_sample_description_index=%d",
     p_track->i_chunk, p_track->chunk[1].i_sample_description_index);
    ...
    {
        msg_DbgVerbose(NULL, "MP4_TrackCreate es has not null");
    }
...
static int MP4_AudioTrackPartInit( demux_t *p_demux, mp4_track_t *p_track,
                             ...
                             bool b_force_enable )
{
    msg_DbgVerbose(NULL, "enter into %s.", __FUNCTION__);

    demux_sys_t *p_sys = p_demux->p_sys;

    p_track->p_track = p_box_trak;

    MP4_Box_t *p_childBox = p_box_trak->p_childBox;
    MP4_Box_t *p_tkhd = MP4_BoxGet( p_box_trak, "tkhd" );
    MP4_Box_t *p_tref = MP4_BoxGet( p_box_trak, "tref" );
    MP4_Box_t *p_elst;
    ...
    if( !p_tkhd )
    {
        msg_DbgVerbose(NULL, "play_stat track do not has tkhd box");
        return VLC_EGENERIC;
    }
    ...
    if( p_tref == NULL)
    {
       msg_DbgVerbose( NULL, "play_stat unhandled box: tref --> FIXME" );
    }

    MP4_Box_t *tmp = NULL;
    p_mdia = MP4_BoxGet( p_box_trak, "mdia");
    if (p_mdia == NULL)
    {
        tmp = p_childBox;
        while (tmp->p_father != NULL && tmp->i_type != ATOM_mdia)
        {
            tmp = tmp->p_father;
        }
        if (tmp->i_type == ATOM_mdia)
        {
            p_mdia = tmp;
        }
        else
        {
            msg_DbgVerbose(NULL, "play_stat track do not has mdia box");
            return VLC_EGENERIC;
        }
    }

    p_minf = MP4_BoxGet( p_mdia, "minf");
    if (p_minf == NULL)
    {
        tmp = p_childBox;
        while (tmp->p_father != NULL && tmp->i_type != ATOM_minf)
        {
            tmp = tmp->p_father;
        }
        if (tmp->i_type == ATOM_minf)
        {
            p_minf = tmp;
        }
        else
        {
            msg_DbgVerbose(NULL, "play_stat track do not has minf box");
            return VLC_EGENERIC;
        }
    }

    p_stbl = MP4_BoxGet( p_mdia, "stbl");
    if (p_stbl == NULL)
    {
        tmp = p_childBox;
        while (tmp->p_father != NULL && tmp->i_type != ATOM_stbl)
        {
            tmp = tmp->p_father;
        }
        if (tmp->i_type == ATOM_stbl)
        {
            p_stbl = tmp;
        }
        else
        {
            msg_DbgVerbose(NULL, "play_stat track do not has stbl box");
            return VLC_EGENERIC;
        }
    }
    p_track->p_stbl = p_stbl;

    p_mdhd = MP4_BoxGet( p_mdia, "mdhd" );
    p_hdlr = MP4_BoxGet( p_mdia, "hdlr" );

    if( ( !p_mdhd )||( !p_hdlr ) )
    {
        msg_DbgVerbose(NULL, "track do not has mdhd or hdlr box");
        return VLC_EGENERIC;
    }
    ...
    {
        case( ATOM_soun ):
            if( !( p_smhd = MP4_BoxGet( p_minf, "smhd" ) ) )
            {
                msg_DbgVerbose(NULL, "play_stat track do not has smhd box");
                return VLC_EGENERIC;
            }
            p_track->fmt.i_cat = AUDIO_ES;
            break;
        ...
        case( ATOM_vide ):
            if( !( p_vmhd = MP4_BoxGet( p_minf, "vmhd" ) ) )
            {
                msg_DbgVerbose(NULL, "play_stat track do not has vmhd box");
                return VLC_EGENERIC;
            }
            p_track->fmt.i_cat = VIDEO_ES;
            break;
    ...
    p_track->i_elst = 0;
    p_track->i_elst_time = 0;
    if( p_track->p_elst == NULL && ( p_track->p_elst = p_elst = MP4_BoxGet( p_box_trak, "edts/elst" ) ) )
    {
        MP4_Box_data_elst_t *elst = BOXDATA(p_elst);
        unsigned int i;
...
/*  TODO
    ...
    p_dinf = MP4_BoxGet( p_minf, "dinf" );
*/
    if( /*!( p_track->p_stbl = MP4_BoxGet( p_minf,"stbl" ) ) ||*/
        !( p_track->p_stsd = MP4_BoxGet( p_stbl,"stsd") ) )
    {
        msg_DbgVerbose(NULL, "track do not has stsd box");
        return VLC_EGENERIC;
    }

    /* Set language */
    if( p_track->fmt.psz_language == NULL && *language && strcmp( language, "```" ) && strcmp( language, "und" ) )
    {
        p_track->fmt.psz_language = strdup( language );
    }
    ...
    if( p_udta )
    {
                 ...
                 p_box_iter = p_box_iter->p_next )
        {
            switch( p_box_iter->i_type )
            {
                case ATOM_0xa9nam:
                case ATOM_name:
                    if (p_track->fmt.psz_description == NULL)
                    {
                        p_track->fmt.psz_description =
                        strdup( p_box_iter->data.p_string->psz_text );
                    }
...
#define MAX_SELECTABLE (INT_MAX - ES_PRIORITY_SELECTABLE_MIN)
    ...
    if (1)
    {
        msg_DbgVerbose(NULL, "enter into MP4_AudioTrackPartInit");

        unsigned int i_sample_description_index = 1;
        ...
        msg_DbgVerbose(NULL, "MP4_AudioTrackPartInit create es_out_Add");
                          ...
                          uint64_t i_pos )
{
    ...
    /* we start from sample 0/chunk 0, hope it won't take too much time */
    /* *** find good chunk *** */
    if( i_pos < p_track->chunk[0].i_offset)
    {
        i_chunk = 0;
    }
    else
    {
        for( i_chunk = 0; ; i_chunk++ )
        {
            ...
            if( i_pos >= p_track->chunk[i_chunk].i_offset &&
                i_pos <  p_track->chunk[i_chunk + 1].i_offset )
            {
                break;
            }
        }
    ...
    uint64_t currPos = p_track->chunk[i_chunk].i_offset;

    msg_DbgVerbose(NULL, "MP4_TrackSeekByPos i_sample_size=%lu i_chunk=%d i_offset=%llu i_sample_description_index=%lu i_sample_count=%lu \
     i_sample_first=%lu i_sample=%lu currPos=%llu i_first_dts=%lu i_pos=%lld",
    ...
    msg_DbgVerbose(NULL, "MP4_TrackSeekByPos i_chunk=%d p_sample_count_dts=%d p_sample_delta_dts=%d i_stts_entry_count=%u",
         i_chunk,
         p_track->chunk[i_chunk].p_sample_count_dts[0],
         p_track->chunk[i_chunk].p_sample_delta_dts[0],
         p_track->chunk[i_chunk].i_stts_entry_count);

    int sample_index = i_sample;
    int i = 0;
    if( p_track->i_sample_size )
    {
        uint32_t i_sample_size = 0;
        ...
        {
            i_sample_size = p_track->i_sample_size;
        }
        else
        {
            i_sample_size = p_soun->i_sample_per_packet * p_soun->i_bytes_per_frame;
        }

        int sample_offset = (i_pos - currPos) / i_sample_size;

        msg_DbgVerbose(NULL, "MP4_TrackSeekByPos sample_offset=%d i_sample_size=%lu p_soun->i_sample_per_packet=%lu i_bytes_per_frame=%lu",
         sample_offset, i_sample_size, p_soun->i_sample_per_packet, p_soun->i_bytes_per_frame);

        if (sample_offset >= p_track->chunk[i_chunk].i_sample_count)
        {
            i_chunk += 1;
            i_sample = p_track->chunk[i_chunk].i_sample_first;
        }
    ...
    {
        for (i = 0; i < p_track->chunk[i_chunk].i_sample_count; i++)
        {
            msg_DbgVerbose(NULL, "MP4_TrackSeekByPos sample_index=%d currPos=%lld", sample_index, currPos);

            if (i_pos <= currPos)
            {
                break;
            }
            currPos += p_track->p_sample_size[sample_index++];
        }

        if (i == p_track->chunk[i_chunk].i_sample_count && i_chunk + 1 < p_track->i_chunk_count)
        {
            i_chunk += 1;
            i_sample = p_track->chunk[i_chunk].i_sample_first;
        }
    ...
    msg_DbgVerbose(NULL, "MP4_TrackSeekByPos i_cat=%d i_chunk=%u i_sample=%u p_track->p_elst=%p", p_track->fmt.i_cat, i_chunk, i_sample, p_track->p_elst);
                          ...
                          mtime_t i_start )
{
	...
	msg_DbgVerbose(NULL, "MP4_TrackSeek i_cat=%d i_chunk=%u i_sample=%u p_track->p_elst=%p", p_track->fmt.i_cat, i_chunk, i_sample, p_track->p_elst);
    p_track->b_selected = true;
    if( !TrackGotoChunkSample( p_demux, p_track, i_chunk, i_sample ) )
        p_track->b_selected = true;
...
static bool AddFragment( demux_t *p_demux, MP4_Box_t *p_moox )
{
    ...
    msg_DbgVerbose(NULL, "AddFragment. type=%lu", p_moox->i_type);
    if( p_moox->i_type == ATOM_moov )
    {
        if ( !p_sys->moovfragment.p_moox )
        {
            ...
            { /* duration might be wrong an be set to whole duration :/ */
                msg_DbgVerbose(NULL, "AddFragment duration might be wrong an be set to whole duration");
               MP4_Box_t *p_tkhd;
