...
int MP4_ReadBoxCommon( stream_t *p_stream, MP4_Box_t *p_box )
{
	...
	dumpMemory(p_peek,32,"MP4_ReadBoxCommon");
    ...
    if( p_box->i_size < stream_Size( p_stream ))
    {
        if MP4_BOX_TYPE_ASCII()
            msg_Dbg( p_stream, "play_stat found Box: %4.4s size %"PRId64" %"PRId64,
                    (char*)&p_box->i_type, p_box->i_size, p_box->i_pos );
        else
            msg_Dbg( p_stream, "play_stat found Box: c%3.3s size %"PRId64,
...
static int MP4_NextBox( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    if( p_box->p_father &&
        ...
        p_box->i_type == ATOM_mdat )
    {
        if (MP4_BoxGet( p_box->p_father, "moov" ) == NULL && p_box->p_father->boxType == MP4_BOX_CUSTOME_TYPE_NORMAL && p_box->p_father->p_stream == NULL)
        {
            if (strcasecmp(p_stream->psz_access, "http") == 0)
            {
                char *url;
                if (asprintf(&url, "%s://%s&startRange=%llu&moovtype=%d", p_stream->psz_access, p_stream->psz_path, (uint64_t)(p_box->i_size + p_box->i_pos), 2) != -1)
                {
                    int64_t cost = mdate();
                    p_box->p_father->p_stream = stream_UrlNew(p_stream, url);
                    if (p_box->p_father->p_stream != NULL)
                    {
                        p_box->p_father->stream_set_flag = true;
                        p_box->p_father->moovBoxType = MP4_BOX_MOOV_POS_END;
                        if (p_box->p_father->box_sys.fun != NULL)
                        {
                            p_box->p_father->box_sys.fun(p_box->p_father->box_sys.priv, p_box, MP4_BOX_INIT_TYPE_DATA);
                        }

                        msg_DbgVerbose(NULL, "MP4_NextBox play_stat_start new stream");
                    }
                    p_box->p_father->stream_new_cost = mdate() - cost;
                    msg_DbgVerbose(NULL, "MP4_NextBox play_stat_start stream url new url=%s p_box->i_size=%llu + p_box->i_pos=%lld.  ", url, p_box->i_size, (long long)(p_box->i_pos));
        ...
        {
            msg_DbgVerbose(NULL, "MP4_NextBox play_stat_start moov is not at end.");
        }
    }

    if (p_box->i_type == ATOM_stsd)
    {
        MP4_Box_t *tmp = p_box;
        while (tmp->p_father != NULL)
        {
            tmp = tmp->p_father;
        }
        if (tmp->i_type == ATOM_root && tmp->stream_set_flag && tmp->moovBoxType == MP4_BOX_MOOV_POS_FRONT)
        {
            if (tmp->box_sys.fun != NULL)
            {
                msg_DbgVerbose(NULL, "play_stat_start: ATOM_stsd");

                tmp->box_sys.fun(tmp->box_sys.priv, p_box, MP4_BOX_INIT_TYPE_DATA);
            }
        }
    }

    if (p_box->i_type == ATOM_trak)
    {
        msg_DbgVerbose(NULL, "play_stat_start p_stream trak stream pos=%lld", stream_Tell( p_stream ));
        MP4_Box_t *tmp = p_box;
        while (tmp->p_father != NULL)
        {
            tmp = tmp->p_father;
        ...
        if (tmp->i_type == ATOM_root)
        {
            //if (MP4_BoxGet( tmp, "/moov/trak[%d]", 0 ) == NULL)
            {
                if (tmp->stream_set_flag && tmp->box_sys.fun != NULL)
                {
                    tmp->box_sys.fun(tmp->box_sys.priv, p_box, MP4_BOX_INIT_TYPE_ADD_TRACK);
                    //msg_DbgVerbose(NULL, "MP4_NextBox play_stat add track. %p", p_box);
                }
            }
        }
    }

    if (p_box->i_type == ATOM_mvhd)
    {
        MP4_Box_t *tmp = p_box;
        while (tmp->p_father != NULL)
        {
        ...
        if (tmp->i_type == ATOM_root)
        {
            if (tmp->stream_set_flag && tmp->box_sys.fun != NULL)
            {
                tmp->box_sys.fun(tmp->box_sys.priv, p_box, MP4_BOX_INIT_TYPE_INIT_MVHD);
            }
        }
    }
    ...
    if (p_box->i_type == ATOM_stsd)
    {
        MP4_Box_t *p_minf = p_box;
        while (p_minf->p_father != NULL && p_minf->i_type != ATOM_minf)
        {
            p_minf = p_minf->p_father;
        }
        if (p_minf != NULL && p_minf->i_type == ATOM_minf)
        {
            MP4_Box_t *p_smhd = MP4_BoxGet( p_minf, "smhd" );
            if (p_smhd != NULL)
            {
                msg_DbgVerbose(NULL, "play_stat smhd ");

                MP4_Box_t *trak = p_box;
                while (trak->p_father != NULL && trak->i_type != ATOM_trak)
                {
                    trak = trak->p_father;
                }

                if (trak->i_type == ATOM_trak)
                {
                    MP4_Box_t *root = trak;
                    while (root->p_father != NULL)
                    {
                        root = root->p_father;
                    }

                    if (root->i_type == ATOM_root)
                    {
                        if (root->stream_set_flag && root->box_sys.fun != NULL)
                        {
                            trak->p_childBox = p_box;
                            root->box_sys.fun(root->box_sys.priv, trak, MP4_BOX_INIT_TYPE_INIT_MP4A_STSD);
                        }
                    }
                }
            }
...
static MP4_Box_t *MP4_ReadBox( stream_t *p_stream, MP4_Box_t *p_father )
{
    ...
    if (p_box->i_type == ATOM_moov && p_box->p_father->boxType == MP4_BOX_CUSTOME_TYPE_NORMAL && p_box->p_father->p_stream == NULL)
    {
        if (strcasecmp(p_stream->psz_access, "http") == 0 && p_box->i_size >= MP4_BOX_MOOV_SIZE_THRESHOLD)
        {
            char *url;
            if (asprintf(&url, "%s://%s&startRange=%llu&moovtype=%d", p_stream->psz_access, p_stream->psz_path, (uint64_t)(p_box->i_size + p_box->i_pos), 1) != -1)
            {
                p_box->p_father->stream_set_flag = true;
                p_box->p_father->moovBoxType = MP4_BOX_MOOV_POS_FRONT;
                p_box->p_father->new_stream_url = url;
                ...
                //     tmp = tmp->p_father;
                // }
                // if (tmp->i_type == ATOM_root && tmp->box_sys.fun != NULL)
                // {
                //     tmp->box_sys.fun(tmp->box_sys.priv, p_box, MP4_BOX_INIT_TYPE_DATA);
                // }

                msg_DbgVerbose(NULL, "MP4_ReadBox play_stat url=%s p_box->i_size=%llu + p_box->i_pos=%lld.  ", url, p_box->i_size, (long long)(p_box->i_pos));
        ...
        {
            msg_DbgVerbose(NULL, "box ATOM_moov i_size=%llu", p_box->i_size);
        }
    }
...
MP4_Box_t *MP4_BoxGetNextChunk( stream_t *s )
{
    ...
    else if( p_tmp_box->i_type == ATOM_ftyp )
    {
        free( p_tmp_box );
        MP4_Box_t *root;
        return MP4_BoxGetRoot( s , &root, NULL);
    }
    free( p_tmp_box );
 ...
 *****************************************************************************/
MP4_Box_t *MP4_BoxGetRoot( stream_t *s , MP4_Box_t **root, MP4_Box_sys_t *box_sys)
{
    ...
    p_root->p_last      = NULL;
    p_root->p_next      = NULL;
    p_root->boxType = MP4_BOX_CUSTOME_TYPE_NORMAL;
    p_root->p_stream = NULL;
    p_root->stream_set_flag = false;
    p_root->new_stream_url = NULL;
    p_root->stream_read_size = 0;
    p_root->stream_read_cost = 0;
    p_root->stream_peek_cost = 0;
    p_root->stream_seek_cost = 0;
    p_root->stream_parse_cost = 0;
    p_root->stream_new_cost = 0;
    if (box_sys != NULL)
    {
        p_root->box_sys = *box_sys;
    }
    ...
    *root = p_root;
    ...
    msg_DbgVerbose(NULL, "MP4_BoxGetRoot play_stat i_result=%d", i_result);

    if (MP4_BoxGet( p_root, "moov" ) == NULL && p_root->boxType == MP4_BOX_CUSTOME_TYPE_NORMAL && p_root->p_stream != NULL)
    {
        return p_root;
    }
	...
	msg_DbgVerbose(NULL, "play_stat MP4_BoxGetRoot p_root->i_size=%llu pos=%lld size=%lld", p_root->i_size, stream_Tell( s ), stream_Size( s ));
...
MP4_Box_t *MP4_BoxParse( MP4_Box_t *p_root )
{
    stream_t *p_stream;
    int i_result;
    ...
    if( p_root == NULL )
    {
        msg_DbgVerbose(NULL, "error, mp4_boxparse failed");

        return NULL;
    }

    msg_Dbg(p_root->p_stream, "play_stat enter into %s", __FUNCTION__);

    p_stream = p_root->p_stream;

    /* First get the moov */
    i_result = MP4_ReadBoxContainerChildren( p_stream, p_root, ATOM_moov );

    msg_Dbg(p_root->p_stream, "play_stat MP4_BoxParse i_result=%d", i_result);
    ...
    msg_Dbg(p_root->p_stream, "play_stat MP4_BoxParse p_root->i_size=%llu pos=%lld size=%lld", p_root->i_size, stream_Tell( p_stream ), stream_Size( p_stream ));
    if( stream_Tell( p_stream ) + 8 < stream_Size( p_stream ) )
    {
        /* Get the rest of the file */
        i_result = MP4_ReadBoxContainerRaw( p_stream, p_root );
    ...
    msg_Dbg(p_root->p_stream, "play_stat MP4_BoxParse ret");
...
int MP4_FindBoxPos( stream_t *p_stream , uint32_t finder_box)
{
    msg_DbgVerbose(NULL, "play_stat enter into %s", __FUNCTION__);
    ...
    while (1)
    {
        memset(p_box, 0, sizeof(MP4_Box_t));
        ...
        msg_DbgVerbose(NULL, "play_stat MP4_FindBoxPos %4.4s", (char*)&p_box->i_type);
        if (p_box->i_type != finder_box)
        {
            if( stream_Seek( p_stream, p_box->i_size + p_box->i_pos) )
            {
                free( p_box );
