...
enum mp4_box_custom_type
{
    MP4_BOX_CUSTOME_TYPE_OTHER = 0,
    MP4_BOX_CUSTOME_TYPE_NORMAL,
};

#define MP4_BOX_MOOV_SIZE_THRESHOLD 156000
enum mp4_box_moov_categary
{
    MP4_BOX_MOOV_POS_FRONT = 0,
    MP4_BOX_MOOV_POS_END,
};

typedef struct MP4_Box_s MP4_Box_t;
typedef int (*mp4_Funtype)(demux_t *p_demux, MP4_Box_t *p_trak, int type);
/* the most basic structure */

enum mp4_box_init_type
{
    MP4_BOX_INIT_TYPE_INIT_MVHD,
    MP4_BOX_INIT_TYPE_ADD_TRACK,
    MP4_BOX_INIT_TYPE_INIT_MP4A_STSD,
    MP4_BOX_INIT_TYPE_DATA,
};

typedef struct MP4_Box_sys_t
{
    void *priv;
    mp4_Funtype fun;
} MP4_Box_sys_t;
...
struct MP4_Box_s
{
    ...
    int boxType;
    int moovBoxType;
    bool stream_set_flag; //should has other stream.
    MP4_Box_t *p_childBox;
    int64_t stream_parse_cost;
    int64_t stream_read_cost;
    int64_t stream_read_size;
    int64_t stream_peek_cost;
    int64_t stream_seek_cost;
    int64_t stream_new_cost;
};
...
#define MP4_READBOX_ENTER( MP4_Box_data_TYPE_t ) \
    ...
    MP4_Box_t *p_root = p_box; \
    while (p_root->p_father != NULL) \
    {                                \
        p_root = p_root->p_father;   \
    }                                \
    int64_t cost = mdate();          \
    i_actually_read = stream_Read( p_stream, p_peek, i_read ); \
    if (p_root != NULL && p_root->i_type == ATOM_root)        \
    {                                       \
        p_root->stream_read_size += i_read;   \
        p_root->stream_read_cost += mdate() - cost;\
    }                                       \
                                            \
    if( i_actually_read < 0 || (int64_t)i_actually_read < i_read )\
    { \
        msg_Warn( p_stream, "%4.4s MP4_READBOX_ENTER: I got %i bytes, "\
        "but I requested %" PRId64, (char*)&p_box->i_type, i_actually_read, i_read );\
        free( p_buff ); \
        return( 0 ); \
    } \
...
#define MP4_READBOX_EXIT( i_code ) \
    ...
    { \
        ...
        p_root->stream_parse_cost += mdate() - cost; \
        return( i_code ); \
    } while (0)
 ...
 *  level boxes
 *****************************************************************************/
MP4_Box_t *MP4_BoxGetRoot( stream_t * , MP4_Box_t **, MP4_Box_sys_t *box_sys);
